...
C 2020.00 01/20/20 Implemented IGRF-13 constants (Simone Maldera)
C 2024.00 11/22/25 Implemented IGRF-14 constants (Nicola Omodei)
...
C ### FILMOD, DTEMOD arrays +1
        CHARACTER*12    FIL1
        DIMENSION       GH1(225),GH2(225),GHA(225),DTEMOD(9)
        ...
        DATA   DTEMOD /1990.,1995.,2000.,2005.,2010.,2015.,2020.,
     n                 2025.,2030./      
C
...
C-- GET IGRF COEFFICIENTS FOR THE BOUNDARY YEARS
	  ...
	  CALL GETSHC (dgrf10, NMAX1, ERAD, GH1, IER)  
          CALL GETSHC (dgrf15, NMAX2, ERAD, GH2, IER)
          
        ENDIF
        IF (L.EQ.6) THEN
	  CALL GETSHC (dgrf15, NMAX1, ERAD, GH1, IER)  
          CALL GETSHC (dgrf20, NMAX2, ERAD, GH2, IER)  
        ENDIF
        IF (L.EQ.7) THEN
	  CALL GETSHC (dgrf20, NMAX1, ERAD, GH1, IER)  
          CALL GETSHC (igrf25, NMAX2, ERAD, GH2, IER)  
        ENDIF
        IF (L.EQ.8) THEN
	  CALL GETSHC (igrf25, NMAX1, ERAD, GH1, IER)  
          CALL GETSHC (igrf25s, NMAX2, ERAD, GH2, IER)  
        ENDIF
...
C
C NOTE: EREQU AND ERPOL REMOVED ON 02/13/15 BECAUSE NOT PART OF 
C COMMON/GENER/ (THEY WERE CAUSING CRASHES WHEN CALLED BY FASTMON)
C-----------------------------------------------------------------
        COMMON/GENER/   UMR,ERA,AQUAD,BQUAD
        ERA=6371.2
